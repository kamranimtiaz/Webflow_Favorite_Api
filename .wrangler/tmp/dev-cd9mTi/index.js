// .wrangler/tmp/bundle-RbQLsu/checked-fetch.js
...
// src/index.ts
var app = new Hono2();
var mainCollectionId = "651da61a8ac88fc567c0d87b";
var careTypeCollectionId = "651da61a8ac88fc567c0d83a";
...
app.post("/update-favorite-count", async (c) => {
  ...
  try {
    ...
    const currentValue = item.fieldData[field];
    if (typeof currentValue !== "number") {
      return c.json({ error: "Field value is not a number" }, 400);
    }
    const newValue = currentValue + delta;
    if (newValue < 0 || newValue > 100) {
      return c.json({ error: "Value exceeds allowed range" }, 400);
    }
    item.fieldData[field] = newValue;
    const payload = {
      item: {
        id: item.id,
    ...
    const updateResponse = await fetch(
      `https://api.webflow.com/v2/collections/${mainCollectionId}/items/${itemId}`,
      {
        method: "PUT",
        headers: {
          accept: "application/json",
    ...
    const updatedItem = await fetchWebflowItem(mainCollectionId, itemId);
...
// .wrangler/tmp/bundle-RbQLsu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-RbQLsu/middleware-loader.entry.ts
